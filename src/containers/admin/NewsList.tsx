...
import { Data, columns } from '@/config/tableList/newsList';
import { ThunkDispatch } from '@/store';
import { newsListOperations, newsListSelectors } from '@/store/admin/newsList';
import { newsOperations } from '@/store/admin/news';
import { getDateTimeString } from '@/libs/utils/dateFormat';
import NewsListTableRow from '@/components/admin/NewsListTableRow';
...
const getRows = (newsList: ReadonlyArray<News>): Data[] => {
...
const AdminNewsList = () => {
  ...
  const { isFetching, isInitialized, newsList, loadingNewsId } = useSelector(
    newsListSelectors.state,
  );
  ...
  const changeStatus = useCallback(
    (newsId: string, newsStatus: NewsStatus) => {
      dispatch(
        newsOperations.updateStatus(
          newsId,
          newsStatus,
          () => {
  ...
  const rows: Data[] = useMemo(() => getRows(newsList), [newsList]);
  ...
  return (
    <Paper className={classes.root}>
      ...
      <TableContainer className={classes.container}>
        <Table stickyHeader size="small" aria-label="admin-news-list">
          <TableHead>
            <TableRow>
              {columns.map((column) => (
                <TableCell
